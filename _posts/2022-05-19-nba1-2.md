...
a();  // function
```
기존 함수의 정의할 때는 변수를 함수 자체로 표현하는 리터럴 방식을 사용했다. 
<br>
...
```
기존에는 object의 각 value값에 접근하고자 할 때 새로운 변수를 설정해 그 값을 담았었다. 
<br>
...
```
`for`문을 사용하면 기본적으로 코드가 3줄 이상이며, `for`문의 실행 횟수를 위해 array의 length값을 알고 있어야 한다.
<br>

`map`을 사용하면 array의 길이를 알아야 할 필요 없이, 더 짧은 코드로 반복문을 실행할 수 있다. `map`은 array에 사용되는 higher order function으로, parameter로 callback 함수를 받아 주어진 array에 대해 실행시킨다. array와 `map`은 연결 연산자(.)로 연결한다.
```js
let numbers = [1, 2, 3, 4, 5];
numbers.map((value, i) => {
  console.log(`[${i}] ${value}`);
...
처음 `map`을 사용할 때는 아래 코드와 같이 출력 순서와 동일하도록 i, value 순으로 parameter를 pass했는데 두 값이 바뀌어 나왔다.
...
+ '자바스크립트 이메일 정규표현식' 검색해 정규 표현식(regular expression)으로 구현해보기
...
```
정규 표현식(regular expression)에서는 `/ /` 안에 내가 찾고자 하는 패턴을 넣는다. `/ /` 뒤에는 g, i, m 등 여러 flag를 덧붙일 수 있다. 위의 코드에서는 `i` flag(ignore cases)가 사용되었는데, 이는 알파벳 대소문자를 구별하지 않고 패턴을 찾겠다는 뜻이다.
<br>
