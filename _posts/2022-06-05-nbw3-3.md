...
# [웹개발 종합반] 3주 3일차 개발일지(3주 10-12강, 숙제)
...
pymongo에는 `insert`, `find`, `update`, `delete`의 네 가지 기능이 있으며, 각각 데이터 입력, 탐색, 업데이트, 삭제를 담당한다.
<br>
...
```
코드부터 살펴보면, 어떠한 값을 찾아서 `same_ages`라는 변수에 넣은 것을 확인할 수 있다. 이 `same_ages`를 출력해보면, 리스트(array) 안에 조건(`{'age':21}`)을 만족하는 두 딕셔너리(object)가 들어와 있음을 확인할 수 있다. `{'_id': False}`란 Robo 3T에서의 '_id' 속성(아래 사진 참고)은 나타내지 말라는 의미다. '_id' 값은 랜덤으로 부여되는 고유한 값이며, 보통 데이터를 탐색할 때 id는 보이지 않게 처리하는 경우가 많다.
...
# {'name': 'bobby', 'age': 21} {'name': 'jane', 'age': 21}
```
`same_ages`는 리스트(array) 안에 딕셔너리(object)가 있는 구조이므로 반복문을 활용해 각 요소를 한 줄씩 출력할 수도 있다.
