...
```
I learned how to generate random number between 0 and 10. `Math.random()` would generate random number between 0-0.999. I multiplied 11 so that the range would become 0-10.99.
...
Remember, `/d`(d with an escape character) acts as a digit. `\d+` meansone or more digit numbers. the `g` after means global, which searches everywhere in the given variable.
