...
> **Expo 클라이언트 앱에서 화면이 보이지 않는다면?**<br>1) Expo 서버와의 연결이 끊어져 있을 수 있다. 클라이언트 앱을 완전히 종료하고, 다시 켠 후 화면의 QR코드를 스캔하자.<br>2) 컴퓨터의 네트워크와 휴대폰의 네트워크가 동일해야 한다. 즉 두 기기가 같은 와이파이 망에 있어야 한다.<br>3) 브라우저 상의 Tunnel, LAN, Local을 차례로 눌러보며 휴대폰과 연결이 되는 방향을 찾아보자.
